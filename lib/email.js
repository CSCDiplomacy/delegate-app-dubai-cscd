...
// recorded. Not the full itemized-invoice template the client originally
// shared (that one's for a different, pre-selection application-fee flow) —
// client confirmed (2026-08-27) the actual base fees and gave a flat 8%
// tax/processing rate, with only the total shown (no subtotal/tax line
// items), and that the receipt/reference number is just the applicant_id,
// not a separate generated id.
const TIER_LABEL = { partial: 'partial (50%) scholarship', self: 'self-financed', alumni: 'alumni' };
...
async function sendRegistrationReceivedEmail({ email, name, tier, applicantId }) {
  if (!resend || !email) return false;
  const greeting = name || 'Delegate';
  const label = TIER_LABEL[tier] || 'scholarship';
  const baseFee = TIER_BASE_FEE[tier];
  const totalStr = baseFee != null ? (baseFee * (1 + TAX_PROCESSING_RATE)).toFixed(2) : null;
  const totalLine = totalStr
    ? `<p><b>Total paid:</b> $${totalStr}</p>`
    : '';
  const totalLineText = totalStr ? `Total paid: $${totalStr}\n` : '';
  const refLine = applicantId ? `<p>Reference: <b>${applicantId}</b></p>` : '';
  const refLineText = applicantId ? `Reference: ${applicantId}\n` : '';
  try {
    const { error } = await resend.emails.send({
      from: `CSCD <${FROM_EMAIL}>`,
      to: email,
      subject: `Registration received — ${EVENT_NAME}`,
      html: `<p>Hi ${greeting},</p><p>We've received your ${label} registration submission for ${EVENT_NAME}. There's nothing further you need to do right now — our team will confirm your place shortly.</p>${refLine}${totalLine}<p>Portal: <a href="${PORTAL_URL}">${PORTAL_URL}</a></p><p>— The CSCD Team</p>`,
      text: `Hi ${greeting},\n\nWe've received your ${label} registration submission for ${EVENT_NAME}. There's nothing further you need to do right now — our team will confirm your place shortly.\n\n${refLineText}${totalLineText}\nPortal: ${PORTAL_URL}\n\n— The CSCD Team`,
    });
    if (error) {
...
module.exports = { sendInterviewReceivedEmail, sendCredentialEmail, sendRegistrationReceivedEmail };
