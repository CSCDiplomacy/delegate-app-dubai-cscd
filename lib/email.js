...
// recorded. Deliberately simple copy, not the itemized-invoice template the
// client shared for a different (pre-selection application-fee) flow — that
// template's fee/txnId/receipt-number fields aren't confirmed to map onto
// what these three JotForm forms' Stripe blocks actually hand back in
// rawRequest, and guessing that mapping risks a wrong dollar amount reaching
// a real delegate. Flagged as a follow-up once the field mapping is
// confirmed; this covers "we got it" in the meantime.
const TIER_LABEL = { partial: 'partial (50%) scholarship', self: 'self-financed', alumni: 'alumni' };

async function sendRegistrationReceivedEmail({ email, name, tier }) {
  if (!resend || !email) return false;
  const greeting = name || 'Delegate';
  ...
  try {
    const { error } = await resend.emails.send({
      from: `CSCD <${FROM_EMAIL}>`,
      to: email,
      subject: `We've received your registration — ${EVENT_NAME}`,
      html: `<p>Hi ${greeting},</p><p>We've received your ${label} registration submission for ${EVENT_NAME}. There's nothing further you need to do right now — our team will confirm your place shortly.</p><p>Portal: <a href="${PORTAL_URL}">${PORTAL_URL}</a></p><p>— The CSCD Team</p>`,
