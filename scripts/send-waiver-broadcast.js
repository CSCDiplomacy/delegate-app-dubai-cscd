// Send the partial-waiver invite as a Resend BROADCAST to all self-financed
// delegates who have not registered/paid (result_tier=self, registration_status
// != submitted). Uses the Resend REST API directly (no SDK dependency).
//
// Flow (mirrors scripts/send-broadcast-incomplete.js, REST version):
//   1. query recipients fresh from Supabase (email + first name)
//   2. reuse an existing Resend audience (account is at its 3-audience limit):
//      clear its contacts, then load the recipients
//   3. create the broadcast (branded CSCD template, with {{{RESEND_UNSUBSCRIBE_URL}}})
//   4. send it now
//
//   node scripts/send-waiver-broadcast.js           # real send
//   node scripts/send-waiver-broadcast.js --dry-run # build audience only, no send
//
// Reads RESEND_API_KEY + SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY from
// .env.live (never logged).
...
const BROADCAST_NAME = 'YSF Dubai 2026 - Partial Waiver Invite';

const KEY = envVal('RESEND_API_KEY');
const SB_URL = envVal('SUPABASE_URL');
...
async function rGet(p) {
  const res = await fetch(`${RESEND}${p}`, { headers: rHeaders });
  return { ok: res.ok, status: res.status, body: await res.json().catch(() => ({})) };
}
async function rPost(p, body) {
  const res = await fetch(`${RESEND}${p}`, { method: 'POST', headers: rHeaders, body: JSON.stringify(body || {}) });
  return { ok: res.ok, status: res.status, body: await res.json().catch(() => ({})) };
}
async function rDelete(p) {
  const res = await fetch(`${RESEND}${p}`, { method: 'DELETE', headers: rHeaders });
...
async function recipients() {
  ...
  const res = await fetch(
    `${SB_URL}/rest/v1/delegates?result_tier=eq.self&select=name,email,registration_status`,
    { headers }
  );
  ...
  const out = [];
  for (const d of rows) {
    if ((d.registration_status || 'not_started') === 'submitted') continue;
    if (!d.email) continue;
    const email = d.email.trim().toLowerCase();
    if (seen.has(email)) continue;
    seen.add(email);
    const first = (d.name || '').trim().split(/\s+/)[0] || '';
    out.push({ email, first_name: first });
  }
  return out;
}
...
<body style="margin:0; padding:0; background:#FBFAF6; -webkit-font-smoothing:antialiased; font-family:'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif; color:#241417;">
  <div style="display:none; max-height:0; overflow:hidden; opacity:0; color:#FBFAF6; font-size:1px; line-height:1px;">
    You are invited to request a Partial Waiver covering 50% of your YSF Dubai 2026 self-financed fee. 5 seats, rolling basis. Deadline 3 September 2026, 11:59 PM (GST).
  </div>
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:640px; width:100%; background:#FFFFFF; border:1px solid rgba(26,51,38,.14); border-radius:0; overflow:hidden; box-shadow:0 1px 2px rgba(15,31,24,.06), 0 8px 30px rgba(15,31,24,.08);">
          ...
          <tr>
            <td class="px" style="padding:30px 28px 6px; font-size:14px; line-height:1.65; color:#241417;">
              ...
              <p style="margin:0 0 12px; color:#5B6660;">Dear {{{FIRST_NAME|Delegate}}},</p>
          ...
          <tr>
            <td class="px" style="padding:16px 28px; background:#F4EEEE; border-top:1px solid rgba(26,51,38,.14); font-family:'Inter',sans-serif; font-size:10px; color:#8A7E7A; line-height:1.5;">
              thecscd.org &nbsp;&middot;&nbsp; Youth Strategic Forum, Dubai 2026<br/>
              <a href="{{{RESEND_UNSUBSCRIBE_URL}}}" style="color:#8A7E7A;">Unsubscribe</a>
            </td>
          </tr>
...
const TEXT =
  `Dear {{{FIRST_NAME|Delegate}}},\n\n` +
  `Thank you for your interest in the Youth Strategic Forum (YSF) Dubai 2026.\n\n` +
  `Given the exceptional interest from delegates across many nationalities, we are pleased to invite self-financed delegates to be considered for a Partial Waiver covering 50% of the self-financed participation fee.\n\n` +
  `Limited availability: 5 partial waiver seats, awarded on a rolling basis. Deadline: 3 September 2026, 11:59 PM (Dubai time, GST).\n\n` +
  `How to apply: log in to your delegate portal at ${PORTAL_URL}, open the "Request a Partial Waiver" card, and answer:\n` +
  `1. What unique perspective, impact, and motivation make you a strong candidate for this waiver?\n` +
  `2. How will your participation contribute to the global strategy and cultural diplomacy dialogue?\n\n` +
  `Guidelines: 150 to 200 words per answer.\n\n` +
  `Warm regards,\nBinaymeen A. Wahid\nProgram Coordinator, Department of Programs\nCenter for Strategy and Cultural Diplomacy (CSCD)\ncontact@thecscd.org\n\n` +
...
async function main() {
  ...
  console.log(`Recipients (self, not registered, with email): ${list.length}`);

  // Reuse an existing audience (account is at its 3-audience plan limit).
  const aud = await rGet('/audiences');
  const audiences = (aud.body && aud.body.data) || [];
  if (!audiences.length) {
    console.error('No Resend audiences found; cannot proceed at plan limit.');
    process.exit(1);
  }
  console.log('Audiences:');
  for (const a of audiences) console.log(`  - ${a.name} (${a.id})`);
  if (process.argv.includes('--list')) {
    console.log('--list: no changes made.');
    return;
  }

  // Pick the audience: --audience <id> to target a specific one, else the first.
  const wantId = (process.argv.find((a) => a.startsWith('--audience=')) || '').split('=')[1];
  const audience = wantId ? audiences.find((a) => a.id === wantId) : audiences[0];
  if (!audience) {
    console.error('Requested audience id not found.');
    process.exit(1);
  }
  console.log(`Using audience: ${audience.name} (${audience.id})`);

  // Clear existing contacts.
  const existing = await rGet(`/audiences/${audience.id}/contacts`);
  const contacts = (existing.body && existing.body.data) || [];
  console.log(`Clearing ${contacts.length} existing contacts...`);
  for (const c of contacts) {
    await rDelete(`/audiences/${audience.id}/contacts/${c.id}`);
    await sleep(120);
  }
  ...
  const failed = [];
  for (const r of list) {
    let res = await rPost(`/audiences/${audience.id}/contacts`, {
      email: r.email,
    ...
    if (res.status === 429) {
      await sleep(1500);
      res = await rPost(`/audiences/${audience.id}/contacts`, {
        email: r.email,
        first_name: r.first_name,
        unsubscribed: false,
      });
    ...
    if (res.ok) added++;
      ...
      failed.push(r.email);
      console.error(`  add failed ${r.email}: ${res.status} ${JSON.stringify(res.body)}`);
    }
    await sleep(120); // stay under Resend's rate limit
  }
  if (failed.length) console.error(`\n${failed.length} contacts failed to add.`);
  console.log(`Loaded ${added}/${list.length} contacts into the audience.`);

  // Create the broadcast.
  const created = await rPost('/broadcasts', {
  ...
  if (!created.ok) {
    console.error('Broadcast create failed:', created.status, JSON.stringify(created.body));
    process.exit(1);
  }
  ...
  if (dryRun) {
    console.log('--dry-run: broadcast created but NOT sent. Review it in the Resend dashboard.');
    return;
  }

  const sent = await rPost(`/broadcasts/${broadcastId}/send`, {});
  if (!sent.ok) {
    console.error('Broadcast send failed:', sent.status, JSON.stringify(sent.body));
    process.exit(1);
  }
  console.log('Broadcast SENT:', JSON.stringify(sent.body));
  console.log(`\nDone. Broadcast ${broadcastId} sent to ${added} contacts.`);
}
